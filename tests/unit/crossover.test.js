...
describe('crossover', () => {
  it('testCrossover', async () => {
    ...
    const crossover = competition.getStageByID('C').getGroupByID('CO')
    ...
    assert.equal(competition.getTeamByID('{C:CO:CO1:winner}').getID(), 'TM3')
    assert.equal(competition.getTeamByID('{C:CO:CO1:loser}').getID(), 'TM1')
    assert.equal(competition.getTeamByID('{C:CO:CO2:winner}').getID(), 'TM4')
    assert.equal(competition.getTeamByID('{C:CO:CO2:loser}').getID(), 'TM2')
  })
  ...
  it('testCrossoverGroupsWithoutNames', async () => {
    ...
    const competition = await Competition.loadFromCompetitionJSON(competitionJSON)
    const crossover = competition.getStageByID('C')

    assert.equal(crossover.getGroupByID('CO0').getName(), null)
    assert.equal(crossover.getGroupByID('CO1').getName(), 'Crossover round')
    assert.equal(crossover.getGroupByID('CO2').getName(), null)
  })
})
